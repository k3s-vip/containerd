...
func TestMetadataMarshalUnmarshal(t *testing.T) {
	...
	t.Logf("should be able to do json.marshal and produce default")
	data, err := json.Marshal(meta)
	assert.NoError(err)
	metadataInternal, err := metadataToInternal(*meta, defaultMetadataVersion)
	assert.NoError(err)
	...
	v2meta := internalToMetadata(newVerMeta.Metadata, defaultMetadataVersion)
